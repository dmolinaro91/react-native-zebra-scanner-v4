...
public class BarcodeModule implements EMDKManager.EMDKListener, Scanner.DataListener, Scanner.StatusListener {
        ...
        if (isScannerPresent()) {
            ...
            if (scanner != null) {
                try {
                    ...
                    log("Exception scanner enable: " + e.getMessage());
                }
            } else {
                log("Enable failed: Scanner is null after initialization");
            }
        }
    }
    ...
    public void onData(ScanDataCollection scanDataCollection) {
        try {
            ...
            log("Exception in onData: " + e.getMessage());
        }
    }
    ...
    private void initializeScanner() {
        if (scanner == null) {
            try {
                // Validar que emdkManager no sea null primero
                if (emdkManager == null) {
                    log("Cannot initialize scanner: emdkManager is null");
                    return;
                }
                
                barcodeManager = (BarcodeManager) emdkManager.getInstance(EMDKManager.FEATURE_TYPE.BARCODE);
                
                // Validar que barcodeManager se creó correctamente
                if (barcodeManager == null) {
                    log("Failed to get BarcodeManager instance");
                    return;
                }
                
                scanner = barcodeManager.getDevice(BarcodeManager.DeviceIdentifier.DEFAULT);

                // Validar que scanner se creó correctamente
                if (scanner == null) {
                ...
                try {
                    ...
                    log("Unexpected error initializing scanner: " + e.getMessage());
                    scanner = null;
                }
            } catch (Exception e) {
                log("Exception in initializeScanner: " + e.getMessage());
                scanner = null;
            }
        }
    }
        ...
        protected void onPostExecute(String barcode) {
            if (barcode != null) {
                try {
                    ...
                    log("Exception in onPostExecute: " + e.getMessage());
                }
            }
        }
    }
